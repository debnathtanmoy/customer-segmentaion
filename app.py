...
type_of_customer_to_recommended_action_mapping = {'New Customer': 'Need to handle with care by improving relationships with them.Company \
                                          ...
                                          'Lost Customer': 'These cutomers may have already exited from the customer base.The company should try to undesratand why they left the system so that it does not happen again.', 
...
def analyse():
    ...
    return render_template('index.html', type_of_customer='Type of customer: {}'.format(customer_info['Type of customer']), recommended_action='Recommended action: {}'.format(customer_info['recommended action']))
